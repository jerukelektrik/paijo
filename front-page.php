...
<main id="main-content">
	...
	<!-- Toko Bercerita Section (Layout: Feed Reels style) -->
	<!-- Toko Bercerita Section (Layout: Feed Reels style) -->
	<?php
	// Query Toko Bercerita from feed_reels post type (pj_feed_category taxonomy)
	$toko_term = get_term_by( 'slug', 'toko-bercerita', 'pj_feed_category' );
	
	if ( $toko_term && ! is_wp_error( $toko_term ) ) :
		$toko_url = get_term_link( $toko_term );
		$toko_query = new WP_Query(
			array(
				...
				'tax_query'           => array(
					array(
						'taxonomy' => 'pj_feed_category',
						'field'    => 'term_id',
						'terms'    => $toko_term->term_id,
					),
				),
			)
		);
			...
			<section class="paijo-section bg-paijo-paper relative overflow-hidden">
				...
				<div class="paijo-container relative z-10">
					...
					<div class="mb-10 text-center">
						...
						<h2 class="text-3xl sm:text-5xl font-sans font-black tracking-tight text-paijo-ink"><?php echo esc_html( $toko_term->name ); ?></h2>
						<p class="mt-3 text-sm sm:text-base text-paijo-muted max-w-2xl mx-auto">
							<?php if ( ! empty( $toko_term->description ) ) : ?>
								<?php echo esc_html( $toko_term->description ); ?>
							<?php else : ?>
								<?php esc_html_e( 'Kisah inspiratif dan menarik dari berbagai tokoh di sekitar kita.', 'paijo' ); ?>
							<?php endif; ?>
						</p>
					</div>
					...
					<div class="relative">
						<!-- Slider Container -->
						<div id="toko-bercerita-slider" class="grid grid-flow-col auto-cols-[82%] md:auto-cols-[46%] lg:auto-cols-[calc(20%-16px)] overflow-x-auto snap-x snap-mandatory gap-5 pb-5 scrollbar-none">
							<?php
							...
							<div class="snap-start flex flex-col bg-white dark:bg-neutral-900 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
								<a href="<?php echo esc_url( $toko_url ); ?>" class="group relative w-full h-[400px] bg-gradient-to-br from-[#050b14] to-[#121b2d] flex flex-col justify-between p-6 overflow-hidden">
									...
									<div class="relative z-10 mt-4">
										...
										<p class="text-xs text-neutral-400 mt-3 leading-relaxed"><?php esc_html_e( 'Temukan lebih banyak video inspiratif di rubrik Toko Bercerita.', 'paijo' ); ?></p>
									</div>
						...
						<button id="toko-bercerita-prev-btn" class="absolute -left-4 top-[200px] -translate-y-1/2 z-20 flex items-center justify-center w-14 h-14 rounded-2xl bg-[#f1818f] text-white hover:bg-[#d96a78] shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer opacity-0 pointer-events-none" aria-label="Previous videos">
							<svg class="w-6 h-6 stroke-current fill-none" stroke-width="3" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
								<polyline points="15 18 9 12 15 6"></polyline>
							</svg>
						</button>

						<button id="toko-bercerita-next-btn" class="absolute -right-4 top-[200px] -translate-y-1/2 z-20 flex items-center justify-center w-14 h-14 rounded-2xl bg-[#f1818f] text-white hover:bg-[#d96a78] shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer" aria-label="Next videos">
							<svg class="w-6 h-6 stroke-current fill-none" stroke-width="3" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round">
								<polyline points="9 18 15 12 9 6"></polyline>
							</svg>
						</button>
					...
					document.addEventListener('DOMContentLoaded', function() {
						const container = document.getElementById('toko-bercerita-slider');
						const prevBtn = document.getElementById('toko-bercerita-prev-btn');
						const nextBtn = document.getElementById('toko-bercerita-next-btn');
	...
	if ( $insight_term && ! is_wp_error( $insight_term ) ) :
		...
		if ( $insight_query->have_posts() ) :
			...
			$insight_featured = $insight_posts[0];
			...
			<section class="paijo-section bg-paijo-paper text-paijo-ink">
				<div class="paijo-container">
					...
					<div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-stretch">
						<a class="group relative min-h-[420px] overflow-hidden bg-paijo-ink text-white sm:min-h-[560px] lg:h-full" href="<?php echo esc_url( get_permalink( $insight_featured ) ); ?>">
							<?php if ( $insight_featured_img ) : ?>
								<img class="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" src="<?php echo esc_url( $insight_featured_img ); ?>" alt="<?php echo esc_attr( get_the_title( $insight_featured ) ); ?>">
							<?php else : ?>
								<div class="absolute inset-0 bg-neutral-900"></div>
							<?php endif; ?>
							<div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent"></div>
							<div class="absolute inset-x-0 bottom-0 z-10 p-6 sm:p-10">
								<p class="paijo-eyebrow mb-3 text-white/80"><?php echo esc_html( paijo_get_category_label( (int) $insight_featured->ID ) ); ?></p>
								<h3 class="max-w-xl text-3xl sm:text-5xl font-serif font-medium leading-tight text-white"><?php echo esc_html( get_the_title( $insight_featured ) ); ?></h3>
								<p class="mt-4 max-w-xl text-sm sm:text-base leading-7 text-white/75 line-clamp-2">
									<?php echo esc_html( paijo_get_card_excerpt( (int) $insight_featured->ID, 24 ) ); ?>
								</p>
								<span class="mt-6 inline-flex border border-white/40 px-7 py-3 text-xs font-black uppercase tracking-[0.14em] text-white transition-colors group-hover:bg-white group-hover:text-black">
									<?php esc_html_e( 'Baca Sekarang', 'paijo' ); ?>
								</span>
						...
						<div class="flex gap-5 overflow-x-auto pb-2 snap-x snap-mandatory lg:grid lg:grid-cols-2 xl:grid-cols-3 lg:overflow-visible lg:pb-0">
							<?php foreach ( $insight_grid_posts as $insight_post ) : ?>
								<?php
								$card_image = paijo_get_thumbnail_url( (int) $insight_post->ID, 'paijo-square' );
								?>
								<article class="group min-w-[38%] snap-start sm:min-w-[30%] lg:min-w-0">
									<a class="block overflow-hidden bg-neutral-200 aspect-square" href="<?php echo esc_url( get_permalink( $insight_post ) ); ?>">
										<?php if ( $card_image ) : ?>
											<img class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" src="<?php echo esc_url( $card_image ); ?>" alt="<?php echo esc_attr( get_the_title( $insight_post ) ); ?>">
										<?php else : ?>
											<div class="flex h-full w-full items-center justify-center bg-neutral-200 text-neutral-400 text-3xl font-black uppercase">
												<?php echo esc_html( mb_substr( get_the_title( $insight_post ), 0, 1 ) ); ?>
											</div>
										<?php endif; ?>
									</a>
									<div class="pt-4">
										<p class="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-paijo-accent"><?php echo esc_html( paijo_get_category_label( (int) $insight_post->ID ) ); ?></p>
										<h3 class="text-base font-serif font-bold leading-snug text-paijo-ink">
											<a class="hover:text-paijo-accent" href="<?php echo esc_url( get_permalink( $insight_post ) ); ?>"><?php echo esc_html( get_the_title( $insight_post ) ); ?></a>
										</h3>
									</div>
								</article>
							<?php endforeach; ?>
