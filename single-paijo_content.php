<?php
/**
 * Single custom post type (paijo_content) template.
 *
 * @package Paijo
 */
			...
			<div class="max-w-3xl mx-auto mt-4">
				...
				<article <?php post_class(); ?>>
					...
					<nav class="flex flex-wrap items-center gap-2 text-[10px] sm:text-xs font-bold text-neutral-400 uppercase tracking-widest mb-6" aria-label="Breadcrumb">
						...
						<?php
						$terms = get_the_terms( get_the_ID(), 'paijo_content_category' );
						$primary_cat = ( ! empty( $terms ) && ! is_wp_error( $terms ) ) ? $terms[0] : null;
						if ( $primary_cat ) :
							$category_label = 'kultur-by-pandangan-jogja' === $primary_cat->slug ? __( 'Kultur', 'paijo' ) : $primary_cat->name;
							?>
							<a href="<?php echo esc_url( get_term_link( $primary_cat ) ); ?>" class="hover:text-paijo-accent transition-colors"><?php echo esc_html( $category_label ); ?></a>
							<span class="text-neutral-300" aria-hidden="true">/</span>
							<?php
						endif;
						?>
					...
					<header class="mb-8">
						...
						<div class="mb-4">
							...
							if ( $primary_cat ) :
								$category_label = 'kultur-by-pandangan-jogja' === $primary_cat->slug ? __( 'Kultur', 'paijo' ) : $primary_cat->name;
								?>
								<a href="<?php echo esc_url( get_term_link( $primary_cat ) ); ?>" class="inline-block paijo-category-capsule px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.15em]">
								...
								<span class="inline-block paijo-category-capsule px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.15em]">
									<?php echo esc_html( paijo_get_content_category_label() ); ?>
								</span>
								<?php
							endif;
							?>
